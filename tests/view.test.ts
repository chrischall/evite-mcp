...
import { viewResponse, EVITE_VIEWS } from '../src/view.js';

const parse = (r: { content: { text: string }[] }) => JSON.parse(r.content[0].text);

describe('the rungs', () => {
  it('offers compact and full, and not raw — full already IS the upstream payload', () => {
    expect(EVITE_VIEWS).toEqual(['compact', 'full']);
  });
...
describe('what compact does — and what it deliberately does not', () => {
  ...
  it('keeps EVERY other field, because nothing here knows which Evite fields matter', () => {
    ...
    const record = {
      id: 'e1', title: 'Party', eventDate: '2026-10-01', rsvpCount: 12, hostName: 'A', cancelledAt: null,
  ...
  it('keeps a page URL', () => {
    const d = { link: 'https://www.evite.com/event/e1' };
