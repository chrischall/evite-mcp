...
// with EVITE_EMAIL + EVITE_PASSWORD set would read and write
// ~/.evite-mcp/session.json — non-hermetic, order-dependent, and able to
...
const CACHE_DIR = mkdtempSync(join(tmpdir(), 'evite-test-cache-'));

beforeEach(() => {
  process.env.EVITE_SESSION_CACHE = 'false';
  process.env.EVITE_SESSION_FILE = join(CACHE_DIR, 'session.json');
});

afterAll(() => {
  rmSync(CACHE_DIR, { recursive: true, force: true });

  const leaked = join(homedir(), '.evite-mcp');
  if (existsSync(leaked)) {
    ...
    throw new Error(
      `A test wrote to ${leaked}. The suite must never touch the real home ` +
        'directory — inject EVITE_SESSION_CACHE=false (or a temp ' +
        'EVITE_SESSION_FILE) into the env that test hands the client.',
    );
  }
});
