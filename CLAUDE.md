...
- **A handler receives arguments the SDK has already validated — do not re-parse them.** Under SDK v2 `registerTool`'s `inputSchema` wants a Zod **object schema** where v1 took a `ZodRawShape`. The raw-shape form is not gone — it survives as a `@deprecated` overload that still compiles and is auto-wrapped with `z.object()` — so this repo wraps every declaration explicitly and that overload is never selected. All 19 are Zod object schemas: the six read tools inline, `inputSchema: z.object({ ...someArgs.shape, view: viewArg() })`, and the thirteen write tools through a named `z.object` const (`rsvpArgs`, `sendMessageArgs`, `eventIdArgs`, …), which carry no `view` at all. The SDK validates against it, rejects out-of-enum values, and applies every `.default()` before the handler runs. Handlers here used to be written `async (raw) => { const args = someArgs.parse(raw); … }`, which re-did that work against a schema that did NOT contain `view` — so the key was silently stripped and the handler had to reach back for it with a cast, `(raw as { view?: string }).view`. Write `async (args) => …` and read `args.view`.
- **Which makes forwarding a deliberate act, so name the fields you send.** That redundant re-parse was accidentally load-bearing: it made a `view` leak structurally impossible, because `view` was gone from `args` before any request could be built. Without it, an `async (args)` handler that ever spreads `args` into a client call would put `view=compact` on the wire — `view` is ours and means nothing to Evite. Every read handler names its fields explicitly, and `tests/tools.test.ts` (`view never reaches Evite`) drives all six read tools on all three rungs and asserts the string never appears in a forwarded call.
