...
- **A handler receives arguments the SDK has already validated — do not re-parse them.** `registerTool`'s `inputSchema` takes a `ZodRawShape`, so `inputSchema: { ...someArgs.shape, view: viewArg() }` is the whole declaration: the SDK builds the object schema from it, rejects out-of-enum values, and applies every `.default()` before the handler runs. Handlers here used to be written `async (raw) => { const args = someArgs.parse(raw); … }`, which re-did that work against a schema that did NOT contain `view` — so the key was silently stripped and the handler had to reach back for it with a cast, `(raw as { view?: string }).view`. Write `async (args) => …` and read `args.view`.
